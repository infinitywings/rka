...
step is to choose one native-manuscript project, review a bounded subset of its
claims, append canonical scopes, and create a minimal claim-sized spine through
the normal revision-guarded interfaces. CPSEval is the smallest current pilot
(81 claims and an existing native manuscript), so it is the recommended case.

This requires PI authorization because it changes live semantic state; the
workbench implementation must not manufacture a positive fixture in a real
project merely to satisfy its own exit gate.
