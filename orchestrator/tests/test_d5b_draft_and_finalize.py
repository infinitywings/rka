...
def test_draft_manifest_emits_manifest_on_decisions_to_present(tmp_root: Path):
    """draft_manifest_node no longer writes files; it emits the manifest
    and env template as structured items on decisions_to_present so the
    PI (or their Claude session) writes them locally."""
    state = _state_for_draft()
    update = O.draft_manifest_node(state, _StubSDK(), _RecordingMCP())
    items = update.get("decisions_to_present", [])
    assert len(items) == 2
    manifest_item = [i for i in items if i["type"] == "manifest"][0]
    env_item = [i for i in items if i["type"] == "env_template"][0]
    assert manifest_item["source_node"] == "draft_manifest"
    assert isinstance(manifest_item["content"], dict)
    assert manifest_item["content"]["manifest_type"] == "baseline"
    tools = manifest_item["content"].get("tools", [])
    assert [t["name"] for t in tools] == ["rka", "context7"]
    assert update.get("draft_manifest_hash")
    assert update.get("draft_manifest_path")
    # env template is a string (not a file path)
    assert isinstance(env_item["content"], str)
    assert env_item["suggested_path"]


def test_draft_manifest_env_template_contains_secrets(tmp_root: Path):
    state = _state_for_draft(
        ratified=[
            {
                "name": "sec-edgar",
    ...
    items = update.get("decisions_to_present", [])
    env_item = [i for i in items if i["type"] == "env_template"][0]
    assert "SEC_EDGAR_API_KEY=<paste-here>" in env_item["content"]
    assert "(required)" in env_item["content"]


def test_draft_manifest_no_secrets_yields_empty_env_template(tmp_root: Path):
    state = _state_for_draft()
    update = O.draft_manifest_node(state, _StubSDK(), _RecordingMCP())
    items = update.get("decisions_to_present", [])
    env_item = [i for i in items if i["type"] == "env_template"][0]
    assert "=<paste-here>" not in env_item["content"]
...
def test_finalize_records_audit_journal_when_all_creds_ok(tmp_root: Path):
    # Set up: draft a manifest (emits content on decisions_to_present),
    # then pass the state to finalize. Use a no-secrets manifest so the
    # probe layer passes trivially.
    state = _state_for_draft()
    draft_update = O.draft_manifest_node(state, _StubSDK(), _RecordingMCP())
    # finalize reads the manifest from decisions_to_present.
    state["decisions_to_present"] = draft_update.get("decisions_to_present", [])
    mcp = _RecordingMCP()
...
def test_finalize_escalates_on_required_secret_missing(tmp_root: Path):
    ...
    draft_update = O.draft_manifest_node(state, _StubSDK(), _RecordingMCP())
    state["decisions_to_present"] = draft_update.get("decisions_to_present", [])
    # No env values provided → required secret missing.
    mcp = _RecordingMCP()
    update = O.finalize_node(state, _StubSDK(), mcp)
...
def test_finalize_audit_summary_omits_secret_values(tmp_root: Path):
    ...
    draft_update = O.draft_manifest_node(state, _StubSDK(), _RecordingMCP())
    state["decisions_to_present"] = draft_update.get("decisions_to_present", [])
    # Write a real-looking value to the .env so credential_validator sees it.
