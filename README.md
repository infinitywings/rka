...
graph TD
    ...
    subgraph "RKA Server"
        MCP["MCP Server — 5 always-on dispatch tools (115 typed operations)"]
...
| Tool             | Purpose                                                                                                                |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `rka_query`      | Dispatcher for **53 read operations** (`status`, `search`, `research_map`, `claim_scope`, `interpretation_candidates`, `list_projects`, `manuscript_context`, `manuscript_reference_manifest`, `manuscript_writing_candidates`, `changes_since`, …) — call with `args={"operation": "…", ...}`. |
| `rka_execute`    | Dispatcher for **62 write/lifecycle operations** (`record_note`, `record_decision`, `create_interpretation_candidate`, `triage_interpretation_candidate`, `set_claim_scope`, `create_mission`, `upsert_argument_spine`, `replace_manuscript_reference_manifest`, `submit_report`, …) — call with `args={"operation": "…", ...}`. |
