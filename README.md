...
**The navigator architecture (v2.6.5+).** Since v2.6.5 the rka MCP server ships **only 3 always-on tools** — the navigator triad `rka_load_tools` / `rka_list_tools` / `rka_help`. The other ~91 tools — `rka_get_status`, `rka_list_projects`, `rka_add_note`, `rka_add_decision`, `rka_create_mission`, `rka_add_literature`, `rka_submit_checkpoint`, `rka_get_journal`, `rka_get_report`, and every other project-scoped lifecycle tool — are **deferred**: present on the server but not advertised until a client registers them via `rka_load_tools(names=[...])`. **Your VERY FIRST tool call in every session is `rka_load_tools(names=[…])`** with your role's typical set. Browse the catalog with `rka_list_tools(category=...)`; inspect a single tool with `rka_help(name=...)`. Both Claude Desktop and Claude Code honor MCP's `notifications/tools/list_changed` event, so a freshly-loaded tool becomes callable mid-session without a reconnect. The Brain / Executor / PI skills shipped under `rka/skills/` know about this and will load the tools they need at session start. Rationale for the 3-tool floor: client-side tool-surface filters (Claude Desktop, others) dropped navigator tools when the always-on tier was 12 in v2.6.3/v2.6.4; cutting to 3 guarantees the navigator survives any reasonable filter.
