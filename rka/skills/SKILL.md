...
Manuscript drafting is intentionally not an RKA Core role. Install and invoke
the independent `rka-writer` plugin when writing support is wanted. Core keeps
research records and provenance available through its public MCP contract.
...
- **v2.7.0+ tool surface.** Five tools are always-on at the MCP server: `rka_query(args)` for reads, `rka_execute(args)` for writes, `rka_describe(operation)` for schema lookup, plus `rka_load_tools` / `rka_help` as escape hatches. Call `rka_describe("")` for the live operation index and counts. The typed Pydantic models behind `rka_query` / `rka_execute` render as `inputSchema.oneOf` with per-branch enum + required-field enforcement, so wrong enum values and missing provenance are rejected at the schema layer BEFORE the call dispatches.
