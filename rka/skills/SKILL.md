...
- **v2.7.0+ tool surface.** Five tools are always-on at the MCP server: `rka_query(args)` for all 38 reads, `rka_execute(args)` for all 49 writes, `rka_describe(operation)` for schema lookup, plus `rka_load_tools` / `rka_help` as escape hatches. The 87 typed Pydantic models behind `rka_query` / `rka_execute` render as `inputSchema.oneOf` with per-branch enum + required-field enforcement, so wrong enum values and missing provenance are rejected at the schema layer BEFORE the call dispatches.
