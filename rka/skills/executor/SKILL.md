...
|---|---|
| `rka_query(args)` | All 60 read operations (missions, status, context, experiments, planning branches, interpretations, claim scope, native manuscripts, change impact, reports, search, etc.) |
| `rka_execute(args)` | All 72 write/lifecycle operations (notes, experiment plans/runs/evidence, planning artifacts, manuscript updates, checkpoints, document ingest, interpretation staging, claim-scope review, etc.) |
...
`args` is a **typed Pydantic model** discriminated by `operation`. FastMCP renders the 132-model union as `inputSchema.oneOf` with per-branch enum + required-field constraints. The schema layer rejects wrong enum values, missing required fields, and missing provenance BEFORE the call is dispatched.
