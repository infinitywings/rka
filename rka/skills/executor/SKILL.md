...
**v2.6.5+ Navigator-only surface**: at session start this MCP server exposes ONLY 3 tools — `rka_load_tools`, `rka_list_tools`, `rka_help`. The other ~91 tools (incl. `rka_get_status`, `rka_get_mission`, `rka_add_note`, `rka_add_decision`, `rka_create_mission`, `rka_add_literature`, `rka_submit_checkpoint`, `rka_submit_report`, etc.) are DEFERRED — present on the server but hidden until you register them.

Your VERY FIRST tool call in every session is `rka_load_tools(names=[…])` with your Executor-role typical set — e.g. `rka_load_tools(names=["rka_get_status", "rka_get_context", "rka_get_mission", "rka_get_checkpoints", "rka_search", "rka_get", "rka_add_note", "rka_submit_checkpoint", "rka_submit_report", "rka_get_journal", "rka_list_projects"])`. This fires `notifications/tools/list_changed`; the tools become callable mid-session. Idempotent.

Browse the deferred catalog with `rka_list_tools(category=…, query=…, tier=…)`; inspect any tool's signature + docstring with `rka_help(name=…)` (works for active or deferred tools).
...
When a workflow step below says "call `rka_get_mission(...)`" or "submit via `rka_submit_report(...)`", ensure the tool is loaded (it will be, if it was in your session-start batch).
