...
(free-text from the most recent checkpoint). Render these so the PI
sees the specific escalation cause before drilling into `errors[]` /
`checkpoints[]`. Absent (null) when the run had no errors/checkpoints —
