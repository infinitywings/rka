...
**Cargo / Tauri builds (release/desktop branch only)**: Tauri 2.x's build scripts scan `target/debug/build/tauri-*/out/permissions/path/autogenerated/` for `.toml` permission files. On volumes without full xattr support (external drives, SMB/AFP mounts, OneDrive/Dropbox/iCloud sync folders), macOS mints `._*.toml` AppleDouble companion files alongside each generated file, which Tauri's scanner picks up and fails on (`stream did not contain valid UTF-8`). Even with the maxdepth-2 purge above, the build script re-creates them mid-compile.

Route cargo's target directory off the affected volume before any cargo invocation in `packaging/tauri/`:

```bash
export CARGO_TARGET_DIR="$HOME/.cache/rka-tauri-target"
cargo check       # or `cargo tauri build`, `cargo tauri dev`, etc.
```

`$HOME` lives on the stock APFS boot drive so AppleDouble files don't appear there. See `packaging/README.md` "AppleDouble notes" on the `release/desktop` branch for the full workaround.
