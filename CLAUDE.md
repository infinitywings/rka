...
**Cargo / Tauri builds (release/desktop branch only)**: Tauri 2.x's build scripts scan `target/debug/build/tauri-*/out/permissions/path/autogenerated/` for `.toml` permission files. The FuSpace volume mints `._*.toml` resource forks alongside each generated file, which Tauri's scanner picks up and fails on (`stream did not contain valid UTF-8`). Even with the maxdepth-2 purge above, the build script re-creates them mid-compile.

Route cargo's target directory off FuSpace before any cargo invocation in `packaging/tauri/`:
