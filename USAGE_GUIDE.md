...
> **Tool surface (v2.6.5+).** The rka MCP server ships a **navigator-only architecture**: only **3 always-on tools** at startup — the navigator triad `rka_load_tools` / `rka_list_tools` / `rka_help`. The other ~91 tools — including `rka_get_status`, `rka_list_projects`, `rka_add_note`, `rka_add_decision`, `rka_create_mission`, `rka_add_literature`, `rka_submit_checkpoint`, `rka_get_journal`, `rka_get_report` — are **deferred** and must be registered via `rka_load_tools(names=[...])` before the LLM can call them. **The LLM's very first tool call in every session is `rka_load_tools(names=[…])`** with its role's typical set. Browse the catalog with `rka_list_tools(category=...)`; inspect a single tool with `rka_help(name=...)`. Both Claude Desktop and Claude Code honor `notifications/tools/list_changed`, so newly-loaded tools become callable mid-session. The Brain / Executor / PI skills bundled with RKA know about this and will load the tools they need at session start; this guide notes a few places below where the user prompt has to nudge Claude to load a specific deferred tool. (Rationale for the 3-tool floor: client-side tool-surface filters dropped navigator tools when the always-on tier was 12 in v2.6.3/v2.6.4; cutting to 3 guarantees the navigator survives any reasonable filter.)
