...
**方法**：全部只读诊断 + 一次已授权的 CAREER 回填。所有数字可复现。
...
极端例子：在 rka_development 搜 `"What is RKA role in the research workflow?"`，
...
**为什么这直接对应 "chase own tail"**：在 CAREER 的 15 条 pivot 上，
...
| 项目 | 覆盖率 | | 项目 | 覆盖率 |
|---|---|---|---|---|
| CAREER | 100%（今日回填） | | delaysteer | **0%** (257) |
| Default Project | 100% | | PESOSE | **0%** (132) |
| rka_development | 95.4% | | NIST_curriculum | **0%** (120) |
| Invarllm | 93.9% | | CPSEval | **18.0%** |
| detectability | 89.1% | | new_cybersecurity_course | 42.1% |
| rka-education | 77.8% | | | |
...
包括此前完全为 0 的 delaysteer(257)、PESOSE(132)、NIST_curriculum(120)。
自 2026-05-23 起持续三个月的索引空洞已闭合。

**不要用** `PUT /api/config/embedding` 触发：它会调
`reshape_all_vec_tables_if_needed` **重建所有 vec 表**，抹掉 rka_development
...
|---|---|---|
| `dec_01KMX18FDAMN7T5YVZ7V8HV6R3` | rka_development | "Merged into dec_64Y"（**非法 id**） |
| `dec_01KMX18FDAMN7T5YVZ7V8HV6RB` | rka_development | 提到 `dec_01KKVSAVMPDRWY2P9639PJXJB9`（**库中不存在**） |
| `dec_01KQFECSBAC760FJRXWW5SNKG1` | Invarllm | 空 |
| `dec_01KS9DHX9R576RFHX8Z13N2FZX` | detectability | 空 |
...
另有 50 条跨项目边（多与 Default Project 相关，疑似早期迁移残留）。
...
| `blind_stale_exposure` | 0.067（CAREER） | 0.000 | `currency/runner.py` |
...
| CAREER · `any_status_signal` | False | **True** |
| CAREER · `blind_stale_exposure` | 0.133 | **0.0** |
| rka_development · `current_first_rate` | 0.0（3/3 都搜不到） | **0.667** |
| CAREER · trace_recall / pivot / stale_surfacing | 1.0 / 8-8 / 0 | 1.0 / 8-8 / 0（无退化） |

**要说准确的一点**：CAREER 的 `stale_only` 仍是 2/15——agent 有时仍然只拿到
