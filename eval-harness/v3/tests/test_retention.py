...
async def test_full_context_and_rka_pass_rag_configurable() -> None:
    ...
    try:
        results = await runner.run_000055REDACTED(SCENARIO, ["full_context", "rag", "rka"])
    finally:
        await runner.close()
...
async def test_rka_arm_outage_is_divergence_and_failure() -> None:
    ...
    try:
        results = await runner.run_000055REDACTED(SCENARIO, ["rka"])
    finally:
        await runner.close()
...
async def test_probe_fires_even_when_filler_too_short() -> None:
    ...
    runner = RetentionRunner(_echo)
    results = await runner.run_000055REDACTED(scenario, ["full_context"])
...
# ----------------------------------------------- runner robustness regressions


async def test_completion_failure_is_a_divergence_not_a_crash() -> None:
    """A dead completion must cost one probe, not the whole run.

    Regression: litellm's default 600 s timeout is below the prefill time of a
    long-context probe on local hardware. The original runner let the
    exception escape ``run_000055REDACTED``, so a single slow probe aborted the run
    and — because results were only written at the end — discarded every
    probe already measured.
    """

    def exploding_completer(system: str, prompt: str) -> str:
        raise TimeoutError("Request timed out.")

    runner = RetentionRunner(exploding_completer)
    try:
        results = await runner.run_000055REDACTED(SCENARIO, ["full_context"])
    finally:
        await runner.close()
    ...
    assert any("completion" in d for d in probe["divergences"])
    assert any("TimeoutError" in d for d in probe["divergences"])


async def test_one_failing_arm_does_not_stop_the_others() -> None:
    """The surviving arms still produce data when one backend misbehaves."""
    calls = {"n": 0}

    def flaky(system: str, prompt: str) -> str:
        calls["n"] += 1
        if calls["n"] == 1:  # first arm dies
            raise RuntimeError("backend exploded")
        return prompt  # echo completer for the rest

    runner = RetentionRunner(flaky)
    try:
        results = await runner.run_000055REDACTED(SCENARIO, ["full_context", "rag"])
    finally:
        await runner.close()

    assert [r["arm"] for r in results] == ["full_context", "rag"]
    assert results[0]["passed"] is False and results[0]["divergences"]
    # the echo arm still saw the seeds and scored normally
...
def test_completer_timeout_is_configurable() -> None:
    ...
    from v3.retention.runner import DEFAULT_TIMEOUT_S, make_litellm_completer

    assert DEFAULT_TIMEOUT_S >= 600  # at least litellm's own default
    previous = os.environ.get("RKA_LLM_MODEL")
    os.environ["RKA_LLM_MODEL"] = "openai/dummy-model"
    try:
        assert make_litellm_completer(timeout_s=1.0) is not None
    finally:
        if previous is None:
            os.environ.pop("RKA_LLM_MODEL", None)
        else:
            os.environ["RKA_LLM_MODEL"] = previous
