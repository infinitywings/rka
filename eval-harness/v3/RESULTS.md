...
|---|---|---|---|---|---|---|
| rka_development | 142 | 98.6 % | 93.0 % | 8 | 116 | 2 |
| rka-education | 121 | 100 % | 28.1 % | 87 | 1 | 0 |
| Invarllm | 179 | 100 % | 100 % | 0 | 43 | 0 |
| detectability | 134 | 100 % | 100 % | 0 | 5 | 2 |
| CPSEval | 118 | 100 % | 100 % | 0 | 0 | 0 |
| PESOSE | 81 | 82.7 % | 82.7 % | 0 | 2 | 0 |
| CAREER | 70 | 100 % | 44.3 % | 39 | 3 | 0 |
| delaysteer | 69 | 100 % | 100 % | 0 | 0 | 0 |
| new_cybersecurity_course | 49 | 100 % | 100 % | 0 | 2 | 0 |
| default | 13 | 53.8 % | 53.8 % | 0 | 13 | 1 |
| NIST_curriculum | 0 | — | — | — | 1 | 0 |
...
rka-education (28.1 %) and CAREER (44.3 %) are dragged down purely by
...
legacy NULL rows and spuriously dropped rka-education from 100 % to 95.9 %.
...
|---|---|---|---|
| CAREER | 0.592 | 0.947 | 1.000 |
| rka_development | 0.695 | 0.861 | 1.000 |
...
- **CAREER: 3 of 5** NL scenarios (ranks 1, 1, 2)
- **rka_development: 3 of 9** (ranks 1, 4, 20)
...
1. **Wrong host.** The instance pointed at `http://192.168.86.21:1234` — a
...
> on rka_development, 0.200 → 0.500 on CAREER) as a "semantic lift". That was
...
> rka_development one improved 0.125 → 1.000 while another regressed
> 0.091 → 0.000. And per the backfill gap below, **CAREER has no vector
...
|---|---|---|---|
| rka_development | 2 093 | 1 997 | 95.4 % |
| Invarllm | 1 087 | 1 024 | 94.2 % |
| detectability | 317 | 282 | 89.0 % |
| **CAREER** | **205** | **0** | **0.0 %** |
| **delaysteer** | **257** | **0** | **0.0 %** |
| **PESOSE** | **118** | **0** | **0.0 %** |
...
those projects, including this suite's own CAREER numbers.
...
every `vec_*` table and would have discarded rka_development's 1 997 existing
...
Result: **CAREER 208/208 = 100 %** embedded, 0 failures, ~2 minutes. A
...
ranks the target claim 7th after. `"711,271"` now ranks it 2nd.
...
3. **No backfill after an outage, and no way to trigger one.** Entities created
   ...
   hole in semantic retrieval — this is how CAREER reached 0 % coverage. A
...
(192.168.86.59), so the same model serves all three arms; embeddings served
locally. Corpus: 2 CAREER scenarios, 6 probes, seeds quoted verbatim from the
live project, filler drawn from the PI's other CPS-security projects. The
`rka` arm reads the live CAREER project read-only.
...
> "LLM-GridEval **should not** be positioned as … the adaptive-attacker/EVG
> proof-of-concept half of the CAREER narrative. That older framing is
> superseded."

The model explicitly rejected the stale framing and the substring check
penalised it for naming what it was rejecting. `must_not_include` cannot
...
1. **CAREER has 0 % vector coverage.** All 205 of its entities were created
...
3. **Search returns snippets, not content.** `/api/search` yields
   ...
   "711,271" — so even a correct hit could not satisfy the check. Real Brain
   usage searches *and then fetches* the entity; the arm only does the first
   step.

The target claim `clm_01KXNVS3AQKVBSC4E9KGA9GQHT` is present in `fts_claims`
but has no embedding, and is not returned even for the targeted keyword query
"final CAREER budget direct costs F&A MTDC" — journal entries about the budget
outrank it.
...
> "The provided context does **not** record the exact final active CAREER
> budget total."
...
> "The in-force decision is **dec_01KWFSQ6WWAGZM84P5AJB24R3J**: the primary
> measurement object is failure attribution…"

That is confident, well-cited, and wrong: the decision actually in force is
`dec_01KX6CZ30ZJWC11EST90ZWWE7Z`. A single-query search returns something
...
- Expected-evidence ground truth taken from a CAREER evidence cluster's claims
...
claim spine exists for one CAREER section.
